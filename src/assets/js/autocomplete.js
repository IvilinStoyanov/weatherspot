...
function loadJSON(callback) {
    ...
    xobj.open('GET', 'https://restcountries.eu/rest/v2/all', true); // Replace 'my_data' with the path to your file
...
function autocomplete(inp) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        for (i = 0; i < dataCounties.length; i++) {
            ...
            if (dataCounties[i].name.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                /*create a DIV element for each matching element:*/
                b = document.createElement("DIV");
                /*make the matching letters bold:*/
                b.innerHTML = "<strong>" + dataCounties[i].name.substr(0, val.length) + "</strong>";
                b.innerHTML += dataCounties[i].name.substr(val.length);
                /*insert a input field that will hold the current array item's value:*/
                b.innerHTML += "<input type='hidden' value='" + dataCounties[i].name + "'>";
